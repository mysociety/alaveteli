...
* (If using Docker, type `docker exec -it app /bin/sh`)
* Type `bundle exec rails console` in the root of your Alaveteli installation
* Type `User.find_by(email: 'you@example.com').add_role(:pro_admin)`, replacing
  the email with the email you used during sign up
