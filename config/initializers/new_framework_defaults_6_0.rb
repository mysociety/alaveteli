...
# queues.analysis = :active_storage_analysis
# queues.purge    = :active_storage_purge

# When assigning to a collection of attachments declared via `has_many_attached`
# replace existing attachments instead of appending. Use #attach to add new
# attachments without replacing existing ones.
...
# backwards compatible with earlier Rails versions. If you send mail in the
# background, job workers need to have a copy of MailDeliveryJob to ensure all
# delivery jobs are processed properly. Make sure your entire app is migrated
# and stable on 6.0 before using this setting.
# Rails.application.config.action_mailer.
#   delivery_job = "ActionMailer::MailDeliveryJob"
