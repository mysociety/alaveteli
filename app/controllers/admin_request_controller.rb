...
class AdminRequestController < AdminController
    before_filter :set_info_request, :only => [:edit, :update, :destroy, :move, :generate_upload_url, :hide]
    ...
    def update
        ...
        if @info_request.update_attributes(info_request_params)
            ...
            redirect_to admin_request_url(@info_request), :notice => 'Request successfully updated.'
        else
            render :edit
        end
    end
    ...
    def destroy
        ...
        redirect_to admin_requests_url, :notice => "Request #{@info_request.url_title} has been completely destroyed. Email of user who made request: #{email}"
    end
    ...
    def move
        if params[:commit] == 'Move request to user' && !params[:user_url_name].blank?
            ...
            redirect_to admin_request_url(@info_request), :error => "Please enter the user or authority to move the request to"
        end
    end
    ...
    def info_request_params
        if params[:info_request]
            params[:info_request].slice(:title, :prominence, :awaiting_description, :allow_new_responses_from, :handle_rejected_responses, :tag_string, :comments_allowed)
        else
            {}
        end
    end
