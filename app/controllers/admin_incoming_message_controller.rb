...
class AdminIncomingMessageController < AdminController
    ...
    def update
        ...
        if @incoming_message.update_attributes(incoming_message_params)
            ...
            redirect_to admin_request_url(@incoming_message.info_request), :notice => 'Incoming message successfully updated.'
        else
            render :edit
        end
    end
    ...
    def destroy
        ...
        @incoming_message.info_request.log_event("destroy_incoming",
            { :editor => admin_current_user(), :deleted_incoming_message_id => @incoming_message.id })
        expire_for_request(@incoming_message.info_request)
        redirect_to admin_request_url(@incoming_message.info_request), :notice => 'Incoming message successfully destroyed.'
    end
        ...
        ActiveRecord::Base.transaction do
            for m in message_ids
                ...
                if destination_request.nil?
                    return redirect_to admin_request_url(previous_request), :error => "Failed to find destination request '#{m}'"
                end
