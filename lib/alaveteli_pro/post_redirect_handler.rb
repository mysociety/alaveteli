module AlaveteliPro
  ##
  # Handle post redirects for pro users
  #
  # A hook for us to override certain post redirects for pro users, e.g. if they
  # start making a request, then we realise they're a pro when they log in, so
  # we want to send them into the pro system
  #
  module PostRedirectHandler
    def override_post_redirect_for_pro(uri, post_redirect, user)
      ...
      return uri unless uri =~ /^(\/[a-z]{2})?\/new$/
