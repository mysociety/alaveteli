...
RSpec.describe IncomingMessage do
  ...
  describe '#make_redactions_permanent' do
    ...
    context 'when there are unmasked attachments' do
      let(:unmasked) { incoming_message.foi_attachments.last }

      before do
        allow(incoming_message).to receive(:redacted?).and_return(true)
        unmasked.update(masked_at: nil)
      end

      it 'masks the attachments' do
        expect { subject }.
          to change { unmasked.reload.masked_at }.from(nil)
      end
    end
