...
RSpec.describe FoiAttachment do
  ...
  describe '#unlock' do
    ...
    before do
      ...
      # HACK: FoiAttachment#handle_locked calls this when unlocking to ensure
      # there's a filename. We don't really need to test this here, so just
      # stubbing for now.
      allow(foi_attachment).
        to receive(:mail_attributes).and_return(filename: 'x')
    end
