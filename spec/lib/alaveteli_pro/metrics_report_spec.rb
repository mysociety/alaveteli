...
describe AlaveteliPro::MetricsReport do
  ...
  describe '#report_data' do
    ...
    let(:pro_user1) { FactoryBot.create(:pro_user) }
    let(:pro_user2) { FactoryBot.create(:pro_user) }
    let(:pro_user3) { FactoryBot.create(:pro_user) }

    before do
      2.times { FactoryBot.create(:info_request, user: user) }
      3.times { FactoryBot.create(:info_request, user: pro_user1) }
    ...
    it 'does not calculate Stripe data if pricing is disabled' do
      with_feature_disabled(:pro_pricing) do
        expect(subject).to_not include(:paying_users)
      end
    end

    it 'includes Stripe data if pricing is enabled' do
  ...
  describe '#stripe_report_data' do
    ...
    it 'returns an empty Hash if pricing is disabled' do
      with_feature_disabled(:pro_pricing) do
    ...
    context 'with pricing enabled', feature: :pro_pricing do
      ...
      let!(:unpaid_sub) do
        subscription = Stripe::Subscription.create(customer: customer,
                                                   plan: pro_plan.id)
        StripeMock.mark_subscription_as_unpaid(subscription)
        subscription
      end
      ...
      # in practice, we're not expecting to see this as the account
      # should autocancel, as we've seen in production
      describe 'returning unpaid user data' do
        it 'returns the number of unpaid users' do
          expect(subject[:unpaid_users][:count]).to eq 1
        end

        it 'returns the subscriber ids of unpaid users' do
          expect(subject[:unpaid_users][:subs]).to eq([unpaid_sub.id])
        end
      end

      describe 'returning new Stripe user data' do
        it 'returns the number of new Stripe users' do
          expect(subject[:new_and_returning_users][:count]).to eq 7
        end

        it 'returns the subscriber ids of new Stripe users' do
          expected =
            [ paid_sub.id, paid_annual_sub.id, unpaid_sub.id,
              half_price_sub.id, trial_sub.id, pending_cancel_sub.id,
              past_due_sub.id ]
