  ...
  describe '#apply_masks!' do
    ...
    describe 'when applying censor rules' do
      ...
      it 'should handle multibyte characters in binary file types as binary data' do
        ...
        class_instance.apply_masks!(data, "application/octet-stream", :censor_rules => @censor_rules).should == 'xx mouse'
      end
    ...
    it 'should keep the uncensored original if uncompression of a PDF fails' do
      orig_pdf = load_file_fixture('tfl.pdf')
      pdf = orig_pdf.dup
      class_instance.stub(:uncompress_pdf){ nil }
      class_instance.apply_masks!(pdf, "application/pdf")
      pdf.should == orig_pdf
    end
    ...
    it 'should use the uncompressed PDF text if re-compression of a compressed PDF fails' do
      orig_pdf = load_file_fixture('tfl.pdf')
      pdf = orig_pdf.dup
      class_instance.stub(:uncompress_pdf){ "something about foi@tfl.gov.uk" }
      class_instance.stub(:compress_pdf){ nil }
      class_instance.apply_masks!(pdf, "application/pdf")
    ...
    it 'should replace a simple email address' do
      ...
      class_instance.apply_masks!("the address is test@example.com", 'text/html', {}).should == expected
    end

    it 'should replace a mobile phone number prefixed with "Mobile"' do
      expected = "the mobile is [mobile number]"
      class_instance.apply_masks!("the mobile is Mobile 55555 555555", 'text/html', {}).should == expected
    end

    it 'should replace a mobile phone number prefixed with "Mob Tel"' do
      expected = "the mobile is [mobile number]"
      class_instance.apply_masks!("the mobile is Mob Tel: 55555 555 555", 'text/html', {}).should == expected
    end

    it 'should replace a mobile phone number prefixed with "Mob/Fax:"' do
      expected = "the mobile is [mobile number]"
      class_instance.apply_masks!("the mobile is Mob/Fax: 55555 555555", 'text/html', {}).should == expected
    end

    it "should replace an Alaveteli login link" do
      expected = "the login link is [Alaveteli login link]"
      class_instance.apply_masks!("the login link is http://test.host/c/ekfmsdfkm", 'text/html', {}).should == expected
    end

    it "should replace a https Alaveteli login link" do
      expected = "the login link is [Alaveteli login link]"
      class_instance.apply_masks!("the login link is https://test.host/c/ekfmsdfkm", 'text/html', {}).should == expected
    end
    ...
    it "should apply censor rules to text" do
      ...
      class_instance.apply_masks!("here is a mouse", 'text/html', {:censor_rules => [ censor_rule ]}).should == expected
    end

    it 'should apply extra masks to text' do
      mask = {:to_replace => 'mouse', :replacement => 'cat'}
      expected = "here is a cat"
      class_instance.apply_masks!("here is a mouse", 'text/html', {:masks => [ mask ]}).should == expected
    end
